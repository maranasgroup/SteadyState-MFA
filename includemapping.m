...
%initialization
mapstr = struct('reactants',[],'products',[],'amm',[],'mmm',[],'rS',[],'pS',[]);
...
[p,q] = dmperm(S1);
S1 = S_bal(p,q);
tol = max(size(S1))*eps*norm(S1,'inf');
[R,jb] = rref(S1,tol);
...
N(~vfree,:) = -R(1:sum(~vfree),vfree);
N(vfree,:) = eye(sum(vfree));
vfree = false(size(vfree));
[rind,colind] = find(N);
colind = [logical(diff(colind));true];
rind = rind(colind);
vfree(rind) = true; % free flux identifiers
[~,cperm] = sort(q(rind));
N(q,:) = N(:,cperm);
vfree(q) = vfree;
        ...
        for j1 = 1:length(fluxes(i).mapping.products.name)
            for j2 = 1:length(fluxes(i).mapping.reactants.name)
                if any(any(fluxes(i).mapping.amm{j1,j2}))
                    ...
                    k3 = ismember(fluxes(i).product,fluxes(i).mapping.products.name(j1));
                    jstoic = fluxes(i).pstoic(k3);
                    jstoic = jstoic(1);
